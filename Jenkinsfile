pipeline {
    ...
    environment {
        ...
        IMAGE = "ts4udocker/ts4u-homecare"
                ...
                script{
                     ...
                     if(BRANCH == 'main'){
                         ...
                         withCredentials([string(credentialsId: 'doctl_token', variable: 'DO_API_KEY')]) {
                                ...
                                helm upgrade --install ts4u-homecare ./helm/ts4u-homecare -f ./helm/ts4u-homecare/values.staging.yaml --set image.tag=${BRANCH}-${VERSION}
                                '''
                            }
                            ...
                            withCredentials([string(credentialsId: 'doctl_token', variable: 'DO_API_KEY')]) {
                                ...
                                helm upgrade --install ts4u-homecare-prod ./helm/ts4u-homecare -f ./helm/ts4u-homecare/values.prod.yaml --set image.tag=${BRANCH}-${VERSION}
                                '''
                            }
                        }
                     }
